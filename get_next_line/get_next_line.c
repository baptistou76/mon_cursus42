...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/11/14 18:06:03 by bcaumont          #+#    #+#             */
/*   Updated: 2024/11/16 08:49:56 by bcaumont         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	fd_buffer[BUFFER_SIZE + 1] = {0};
	char		*line;

	if (fd < 0 || BUFFER_SIZE < 0)
		return (NULL);
	line = _fd_reader_to_line(fd, fd_buffer);
	if (line == NULL)
		return (NULL);
	if (ft_strlen(line) == 0)
